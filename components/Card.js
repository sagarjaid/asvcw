...
const Card = ({ pageObj }) => {
  return (
    <div className="flex w-full flex-wrap justify-center gap-2 p-4  py-10">
      {pageObj?.map((el) => {
        return (
          <a
            key={el.name}
            href={`${domainUrl}/${el.url}`}
            className="flex w-72 flex-col items-start justify-start gap-3 rounded-lg border border-stone-200 p-5 text-left hover:border-black"
          >
            <div>
              <svg
